...
class QuizActivity : AppCompatActivity(), View.OnClickListener {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        myAnim = AnimationUtils.loadAnimation(this@QuizActivity, R.anim.dialog_anim_up)
        ...
        viewModel.quizLiveData.observe(this, Observer { quiz ->
            if (quiz[3] == 1) {
                getRandomQuizAdd(quiz[0], quiz[1], quiz[2])
            } else if (quiz[3] == 2) {
                getRandomQuizSub(quiz[0], quiz[1], quiz[2])
            } else if (quiz[3] == 3) {
                getRandomQuizMul(quiz[0], quiz[1], quiz[2])
            } else {
                getRandomQuizDiv(quiz[0], quiz[1], quiz[2])
            }
        })
        ...
        handler.postDelayed({
            ...
            if (!gameClosed) {
                ...
                showGameOverDialog(this@QuizActivity, score, userResult)
            }
        }, 500)
    }
    ...
    private fun getRandomQuizAdd(x: Int, y: Int, z: Int) {
        answerCorrect = false
        countDownTimer?.cancel()
        ...
        if (a == 1) {
            ...
            btn2.text = (z + 6).toString() + ""
            btn3.text = z.toString() + ""
            btn4.text = (z + 5).toString() + ""
        } else {
            btn1.text = (z + 3).toString() + ""
            btn2.text = (z + 4).toString() + ""
            btn3.text = (z + 1).toString() + ""
    ...
    private fun getRandomQuizMul(x: Int, y: Int, z: Int) {
        answerCorrect = false
        countDownTimer?.cancel()
        userResult = z.toString()
        var p = ""
        ...
        if (a == 1) {
            btn1.text = z.toString() + ""
            btn2.text = (z + 2).toString() + ""
            btn3.text = (z + 3).toString() + ""
            btn4.text = (z + 1).toString() + ""
        } else if (a == 2) {
            btn1.text = (z + 4).toString() + ""
            btn2.text = z.toString() + ""
            btn3.text = (z + 1).toString() + ""
    ...
    private fun getRandomQuizDiv(x: Int, y: Int, z: Int) {
        answerCorrect = false
        countDownTimer?.cancel()
        userResult = z.toString()
        var p = ""
    ...
    private fun getRandomQuiz() {
        val a = getRandomNo(4, 1)
        if (a == 1) {
            viewModel.createRandomAdditionQuiz()
        } else if (a == 2) {
            viewModel.createRandomSubtractionQuiz()
        } else if (a == 3) {
            viewModel.createRandomMultiplicationQuiz()
        } else {
            viewModel.createRandomDivisionQuiz()
        }
    }
